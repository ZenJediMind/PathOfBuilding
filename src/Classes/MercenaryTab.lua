...
local function forEachConfigSet(mercenaryTab, fn)
	local configTab = mercenaryTab.build.configTab
	if not configTab then
		return
	end
	for configSetId, configSet in pairs(configTab.configSets) do
		fn(configTab, configSetId, configSet)
	end
end

local function itemSetOwnedByMercenary(mercenaryTab, itemSetId, excludeActive)
	if not itemSetId then
		return false
	end
	for setId, profile in pairs(mercenaryTab.mercenarySets) do
		if profile.itemSetId == itemSetId and (not excludeActive or setId ~= mercenaryTab.activeMercenarySetId) then
			return true
		end
	end
	return false
...
---@class MercenarySkillListControl: ListControl
...
function MercenarySkillListClass:OnHoverKeyUp(key)
	...
	elseif key == "LEFTBUTTON" and IsKeyDown("CTRL") then
		self.mercenaryTab:SetSkillEnabled(skill, skill.enabled == false)
	end
end
...
---@class MercenaryTab: UndoHandler, ControlHost, Control
local MercenaryTabClass = newClass("MercenaryTab", "UndoHandler", "ControlHost", "Control")
...
function MercenaryTabClass:MercenaryTab(build)
	...
	self.controls.setSelect = new("DropDownControl"):DropDownControl({ "LEFT", self.controls.setLabel, "RIGHT" }, { 8, 0, 210, 20 }, { }, function(_, value)
		...
		local alreadyBound = configSet and configSet.mercenarySetId == value.id
		if value.id == self.activeMercenarySetId and alreadyBound then
			return
		end
		self:SetActiveMercenarySet(value.id, false, true)
		self:AddUndoState()
	end)
	...
	self.controls.editEquipment = new("ButtonControl"):ButtonControl({ "TOPLEFT", self.controls.buildLabel, "BOTTOMLEFT" }, { 0, 14, 150, 20 }, "Edit Equipment", function()
		...
		local itemSet = itemsTab:GetActorItemSet("MERCENARY") or self:GetItemSet(true)
		itemsTab:SetViewItemSet(itemSet.id, "MERCENARY")
...
- Ctrl + Right click to include/exclude in Full DPS calculations.
	...
	self.controls.skillEnabled = new("CheckBoxControl"):CheckBoxControl({ "TOPLEFT", self.controls.skillDetailAnchor, "TOPLEFT" }, { 68, 30, 20 }, nil, function(state)
		self:SetSkillEnabled(self.profile.skills[self.selectedSkillIndex], state)
	end)
...
local function calcMercenaryPreview(tab, skillId, calcFunc, useFullDPS)
	if not calcFunc then return nil end
	return MercenaryTools.withMainSkill(tab.profile, skillId, function()
		return calcFunc({ comparisonActor = "MERCENARY" }, useFullDPS)
	end)
end
	...
	if preview then
		...
		if selected then
			local previewSkill = {
				...
				count = 1,
				skillPart = self.build.data.mercenaryStatData.defaultSkillParts[value.id] or 1,
			}
			MercenaryTools.withMainSkill(self.profile, value.id, function()
				MercenaryTools.withReplacedSkill(self.profile, self.selectedSkillIndex, previewSkill, function()
...
function MercenaryTabClass:AddSupportTooltip(tooltip, support, index, preview)
	...
	if preview then
		...
		if selected and selectedData and selectedData.possibleSupportIds then
			...
			local calcFunc = calcTab:GetMiscCalculator()
			if not calcFunc or not calcTab.mainEnv then return end
			local useFullDPS = self.sortGemsByDPSField == "FullDPS"
			local header = "^7Selecting this support will give you:"
			local baseOutput = calcMercenaryPreview(self, selected.id, calcFunc, useFullDPS)
			if not MercenaryTools.mercenaryOutputAvailable(baseOutput) then
				tooltip:AddLine(16, colorCodes.WARNING.."Mercenary comparison unavailable")
				return
			end
			MercenaryTools.withEditedSupports(selected, index, { id = support.id, tier = support.variant }, function()
				local output = calcMercenaryPreview(self, selected.id, calcFunc, useFullDPS)
				if not output then
					tooltip:AddLine(16, colorCodes.WARNING.."Mercenary comparison unavailable")
					return
				end
				self.build:AddStatComparesToTooltip(tooltip, baseOutput, output, header, nil, "MERCENARY")
			end)
		end
	end
end
...
function MercenaryTabClass:Changed()
	...
	self.build.configTab:EnsureHiredMercenaryModList()
	self:InvalidateSupportSort()
	self:RefreshControls()
	self:AddUndoState()
end

function MercenaryTabClass:ItemSetOwnedByAnyMercenary(itemSetId)
	return itemSetOwnedByMercenary(self, itemSetId, false)
end

function MercenaryTabClass:ItemSetOwnedByOtherLoadout(itemSetId)
	return itemSetOwnedByMercenary(self, itemSetId, true)
end

function MercenaryTabClass:ClearConfigBindings(setId)
	if not setId then
		return
	end
	forEachConfigSet(self, function(configTab, _, configSet)
		if configSet.mercenarySetId == setId then
			configSet.mercenarySetId = nil
			configTab.modFlag = true
		end
	end)
end

function MercenaryTabClass:DropInvalidConfigBindings(markDirty)
	forEachConfigSet(self, function(configTab, _, configSet)
		if configSet.mercenarySetId and not self.mercenarySets[configSet.mercenarySetId] then
			configSet.mercenarySetId = nil
			if markDirty then
				configTab.modFlag = true
			end
...
function MercenaryTabClass:GetConfigMercenaryBindings()
	local bindings = { }
	forEachConfigSet(self, function(_, configSetId, configSet)
		-- false records "unbound" so Restore can tell it apart from a Config
		-- that did not exist when this snapshot was taken.
		bindings[configSetId] = configSet.mercenarySetId or false
	end)
	return bindings
end

function MercenaryTabClass:StoreActiveMercenaryItemSet()
	local itemsTab = self.build.itemsTab
	if not itemsTab or not self.profile then
		return
	end
	local itemSetId = itemsTab:GetActorItemSetId("MERCENARY")
	if itemSetId then
		self.profile.itemSetId = itemSetId
	end
end

function MercenaryTabClass:ApplyActiveMercenaryItemSet()
	local itemsTab = self.build.itemsTab
	if not itemsTab or not self.profile then
		return
	end
	local itemSetId = self.profile.itemSetId
	if itemSetId and not itemsTab.itemSets[itemSetId] then
		self.profile.itemSetId = nil
		itemSetId = nil
	end
	itemsTab:SetActorItemSet("MERCENARY", itemSetId, false)
	if itemsTab.viewComparisonActor == "MERCENARY" then
		if itemSetId then
...
function MercenaryTabClass:GetItemSet(create)
	local itemsTab = self.build.itemsTab
	local itemSetId = self.profile and self.profile.itemSetId
	if itemSetId and itemsTab.itemSets[itemSetId] then
		return itemsTab.itemSets[itemSetId]
	end
	if self.profile then
		self.profile.itemSetId = nil
	end
	local actorSet = itemsTab:GetActorItemSet("MERCENARY")
	if actorSet and not self:ItemSetOwnedByOtherLoadout(actorSet.id) then
		if self.profile then
			self.profile.itemSetId = actorSet.id
		end
		return actorSet
	end
	if create == true then
		local itemSet = itemsTab:NewItemSet()
		itemSet.title = (self.profile and self.profile.title) or "Mercenary Equipment"
...
local function copySupportSortList(list)
	local copy = { }
	for i, entry in ipairs(list or { }) do
		copy[i] = copyTable(entry, true)
	end
	return copy
end
...
local function supportDPS(output, field)
	...
	if field == "FullDPS" and output.FullDPS ~= nil then
		return output.FullDPS
	end
	local actor = output.Minion or output
	if actor[field] ~= nil then
		return actor[field]
	end
...
function MercenaryTabClass:ApplySupportSort(index, list)
	...
	control:SetList(copySupportSortList(list))
	local selected = self.profile.skills[self.selectedSkillIndex]
	control:SelByValue(selected and selected.supports[index] and selected.supports[index].id, "id")
end
...
function MercenaryTabClass:QueueSupportSort(index)
	...
	local originalList = copySupportSortList(control.list)
	...
	self.supportSortCoroutine = coroutine.create(function()
		...
		for entryIndex, entry in ipairs(originalList) do
			entry.dps = MercenaryTools.withEditedSupports(selected, index, entry.id and { id = entry.id, tier = entry.tier } or nil, function()
				return supportDPS(calcMercenaryPreview(self, selected.id, calcFunc, useFullDPS), self.sortGemsByDPSField)
			end)
...
function MercenaryTabClass:RefreshControls()
	...
	if not classGroup then
		classGroup = self.classGroups[self.controls.class.selIndex] or self.classGroups[1]
	end
...
-- bindToLoadout defaults false so a forgotten caller cannot rewrite Config.mercenarySetId.
-- Pass true from the Mercenary dropdown and manage-list double-click.
-- Callers that are user actions (dropdown, double-click, import, delete) push undo;
-- loadout switch does not, matching ItemsTab/SkillsTab/ConfigTab SetActive.
function MercenaryTabClass:SetActiveMercenarySet(setId, init, bindToLoadout)
	...
	if self.activeMercenarySetId and self.profile and self.mercenarySets[self.activeMercenarySetId] then
		...
		if not init and self.activeMercenarySetId ~= setId then
			self:StoreActiveMercenaryItemSet()
		end
	end
	if not setId or not self.mercenarySets[setId] then
		setId = self.mercenarySetOrderList[1]
	end
	local switched = self.activeMercenarySetId ~= setId
	if switched then self:InvalidateSupportSort() end

	self.activeMercenarySetId = setId
	self.profile = self.mercenarySets[setId]
	if not init then
		self:ApplyActiveMercenaryItemSet()
		if bindToLoadout then
			local configTab = self.build.configTab
			local configSet = configTab and configTab.configSets[configTab.activeConfigSetId]
			if configSet then
				configSet.mercenarySetId = setId
				configTab.modFlag = true
			end
		end
	end
...
function MercenaryTabClass:SetSkillEnabled(skill, enabled)
	if not skill then return end
	skill.enabled = enabled and true or false
	if skill.enabled == false and self.profile.mainSkillId == skill.id then
		self.profile.mainSkillId = MercenaryTools.firstEnabledSkillId(self.profile)
	elseif skill.enabled and not self.profile.mainSkillId then
...
function MercenaryTabClass:SetSupport(index, supportId)
	...
	if supportId then
		...
		local support = self.data.supports[supportId]
		MercenaryTools.setSkillSupport(skill.supports, index, { id = supportId, tier = support.variant })
	else
		MercenaryTools.setSkillSupport(skill.supports, index, nil)
	end
	self:Changed()
	return true
end
...
function MercenaryTabClass:PlayerFlag(flagName)
	...
	if not (mainEnv and mainEnv.modDB) then
		return nil
	end
	return mainEnv.modDB:Flag(nil, flagName) and true or false
end

function MercenaryTabClass:GetErrors()
	local errors = { }
	local seen = { }
	local function addError(errorText)
		if seen[errorText] then return end
		seen[errorText] = true
		t_insert(errors, errorText)
	end
	local canHire = self:PlayerFlag("CanHirePermanentMercenary")
	if canHire == false then
		addError("Permanently hiring a Mercenary requires Noble Blood, from the Scion's Luminary ascendancy")
	end
	...
	if (self.profile.foundAreaLevel or 0) >= self.build.characterLevel + 20 then
		addError("Character level is at least 20 below the Mercenary level")
	end
	local mercBuild = self.data and self.data.builds[self.profile.buildId]
	if mercBuild and #mercBuild.weaponTypes == 0 then
		addError("Selected build has no exported wieldable-type data")
	end
	local itemsTab = self.build.itemsTab
	...
	-- Construction-time failures only. Profile and current-loadout equipment
	-- errors are produced above; do not echo them from mercenaryCalculationErrors.
	local mainEnv = self.build.calcsTab and self.build.calcsTab.mainEnv
	for _, calculationError in ipairs(mainEnv and mainEnv.mercenaryCalculationErrors or { }) do
		addError(calculationError)
	end
	return errors
end
...
function MercenaryTabClass:Load(xml)
	local function loadSkill(node, profile)
		local skill = {
			...
			count = tonumber(node.attrib.count) or 1,
...
function MercenaryTabClass:PostLoad()
	self:DropInvalidConfigBindings(false)
	local itemsTab = self.build.itemsTab
	if itemsTab then
		for _, profile in pairs(self.mercenarySets) do
			if profile.itemSetId and not itemsTab.itemSets[profile.itemSetId] then
				profile.itemSetId = nil
			end
		end
		if self.profile and not self.profile.itemSetId then
			local fallback = itemsTab:GetActorItemSetId("MERCENARY")
			if fallback and itemsTab.itemSets[fallback] then
				self.profile.itemSetId = fallback
			end
		end
		local previous = itemsTab:GetActorItemSetId("MERCENARY")
		self:ApplyActiveMercenaryItemSet()
		if itemsTab:GetActorItemSetId("MERCENARY") ~= previous then
...
function MercenaryTabClass:CreateUndoState()
	...
	return {
		...
		sortGemsByDPSField = self.sortGemsByDPSField,
		configMercenarySetIds = self:GetConfigMercenaryBindings(),
	}
end

function MercenaryTabClass:RestoreUndoState(state)
	local previousActive = self.activeMercenarySetId
	...
	for index, setId in ipairs(state.mercenarySetOrderList) do
		self.mercenarySetOrderList[index] = setId
	end
	...
	self:InvalidateSupportSort()
	if previousActive ~= self.activeMercenarySetId then
		self:ApplyActiveMercenaryItemSet()
	end
	if state.configMercenarySetIds and self.build.configTab then
		local configTab = self.build.configTab
		for configSetId, configSet in pairs(configTab.configSets) do
			local restoredId = state.configMercenarySetIds[configSetId]
			-- Missing key: Config was created after this snapshot; leave it.
			-- false: was unbound. number: restore that hire.
			if restoredId ~= nil then
				local setId = restoredId or nil
				if configSet.mercenarySetId ~= setId then
					configSet.mercenarySetId = setId
					configTab.modFlag = true
				end
