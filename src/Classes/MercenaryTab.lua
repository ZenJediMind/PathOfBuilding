...
function MercenaryTabClass:Load(xml)
	...
	for _, child in ipairs(xml) do
		if child.elem == "MercenarySet" then
			...
			if setId == activeMercenarySetId and profile.itemSetId then
				self.legacyItemSetId = self.legacyItemSetId or profile.itemSetId
			end
		end
	end
...
function MercenaryTabClass:PostLoad()
	...
	if itemsTab then
		...
		if self.profile and not self.profile.itemSetId then
			local fallback = self.legacyItemSetId or itemsTab:GetActorItemSetId("MERCENARY")
