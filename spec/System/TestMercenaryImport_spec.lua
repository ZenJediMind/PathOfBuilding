describe("Mercenary API import", function()
	...
	it("preserves Guardian equipment referenced by an inactive skill set on reimport", function()
		local profile = import(1)
		local set = build.itemsTab.itemSets[profile.itemSetId]
		local helmetId = set.Helmet.selItemId
		local helmet = build.itemsTab.items[helmetId]
		...
		assert.is_true(build.itemsTab:IsItemSetReferenced(set.id))
		local otherSkills = build.skillsTab:NewSkillSet()
		table.insert(build.skillsTab.skillSetOrderList, otherSkills.id)
		build.skillsTab:SetActiveSkillSet(otherSkills.id)
		assert.is_true(build.itemsTab:IsItemSetReferenced(set.id))
		for index, item in ipairs(roster.mercenaries[1].items) do
			if item.inventoryId == "Helm" then table.remove(roster.mercenaries[1].items, index) break end
		end
		local imported = import(1)
		assert.not_equals(set.id, imported.itemSetId)
		assert.equals(helmetId, set.Helmet.selItemId)
		assert.equals(helmet, build.itemsTab.items[helmetId])
	end)
