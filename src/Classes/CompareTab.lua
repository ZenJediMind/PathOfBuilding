...
local function getActiveItemSetSlot(itemsTab, slotName)
	local itemSet = itemsTab and itemsTab:GetVisibleItemSet()
...
function CompareTabClass:InitControls()
	...
	self.controls.primCalcsShowMinion = new("DropDownControl"):DropDownControl(nil, {0, 0, 140, 18}, {}, function(index, value)
		self.primaryBuild.calcsTab:SetCalculationActor(value.actorId)
		self.primaryBuild.buildFlag = true
	end)
	...
	self.controls.cmpCalcsShowMinion = new("DropDownControl"):DropDownControl(nil, {0, 0, 140, 18}, {}, function(index, value)
		local entry = self:GetActiveCompare()
		if entry then
			entry.calcsTab:SetCalculationActor(value.actorId)
			entry.buildFlag = true
		end
	end)
		...
		if event.type == "KeyUp" and mouseInContent and not mouseOverList then
			...
			elseif event.key == "WHEELDOWN" and self.compareViewMode ~= "TREE" and not mouseOverViewScrollBar then
				...
				elseif self.compareViewMode == "SUMMARY" and self.summaryTotalContentHeight > 0 then
					local scrollViewH = viewportH - LAYOUT.summaryHeaderHeight
				...
				elseif self.compareViewMode == "SKILLS" and self.skillsTotalContentHeight > 0 then
					...
					local scrollViewH = viewportH - LAYOUT.skillsHeaderHeight - hBarReserve
			...
			if cItem and cItem.raw and not (pItem and pItem.name == cItem.name) then
				...
				if newItem.abyssalSocketCount > 0 then
					for idx = 1, newItem.abyssalSocketCount do
						...
						local primarySlot = self.primaryBuild.itemsTab.slots[abyssSlotName]
				...
				if newItem.abyssalSocketCount > 0 then
					for k, v in pairs(oldEquipped) do
						local primarySlot = self.primaryBuild.itemsTab.slots[k]
