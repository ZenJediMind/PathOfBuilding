describe("Mercenary API import", function()
	...
	it("imports mercenary equipment through ItemsTab and preserves player and Guardian data", function()
		...
		assert.equals(secondSetId, second.itemSetId)
		assert.equals(firstSetId, tab.mercenarySets[first.id].itemSetId)
		...
		assert.equals(firstSetId, tab.profile.itemSetId)
	...
	it("persists lower Mercenary levels, associations and equipment through XML and undo", function()
		...
		assert.equals(10, itemCount())
		build.itemsTab:Redo()
		assert.equals(secondSetId, mercSetId())
		assert.equals(97, build.characterLevel)
	...
	it("Mercenary undo restores imported profile without creating or deleting item sets", function()
		...
		assert.are_not.equal(firstSetId, mercSetId())
		assert.equals(firstSetId, first.itemSetId)
		assert.is_not_nil(second.itemSetId)
		tab:Undo()
		assert.equals(firstBuildId, tab.profile.buildId)
		assert.equals(firstSetId, mercSetId())
		assert.are.equal(setCount + 1, #build.itemsTab.itemSetOrderList)
	...
	it("empty Mercenary loadouts do not own item sets", function()
		import(1)
	...
	it("persists each imported hire's equipment through XML and loadout switches", function()
		local first = import(1)
		local second = import(2)
		local firstSetId, secondSetId = first.itemSetId, second.itemSetId
		assert.are_not.equal(firstSetId, secondSetId)
		local firstHelm = build.itemsTab.itemSets[firstSetId].Helmet.selItemId
		local itemsXml, mercXml = { }, { }
		...
		assert.equals(secondSetId, build.itemsTab:GetActorItemSetId("MERCENARY"))
		assert.equals(firstSetId, build.mercenaryTab.mercenarySets[first.id].itemSetId)
		build.mercenaryTab:SetActiveMercenarySet(first.id)
		assert.equals(firstSetId, build.itemsTab:GetActorItemSetId("MERCENARY"))
		assert.equals(firstHelm, build.itemsTab:GetActorItemSet("MERCENARY").Helmet.selItemId)
		assert.is_true(build.itemsTab:IsItemSetReferenced(secondSetId))
	end)
	...
	it("switching Mercenary loadouts restores that hire's own equipment", function()
		helpers.allocatePermanentHire()
		...
		local secondSetId = mercSetId()
		assert.are_not.equal(firstSetId, secondSetId)
		local secondHelm = build.itemsTab.itemSets[secondSetId].Helmet.selItemId
		assert.are_not.equal(firstHelm, secondHelm)
		...
		assert.equals(firstSetId, mercSetId())
		assert.equals(firstHelm, build.itemsTab:GetActorItemSet("MERCENARY").Helmet.selItemId)
		local firstAgain = helpers.calculateBuild()
		assert.equals(firstLife, firstAgain.mercenary.output.Life)
		tab:SetActiveMercenarySet(second.id)
		assert.equals(second.buildId, tab.profile.buildId)
		assert.equals(secondSetId, mercSetId())
		assert.equals(secondHelm, build.itemsTab:GetActorItemSet("MERCENARY").Helmet.selItemId)
	end)

	it("API association includes skill hashes because LeagueAccount has no unique Mercenary id", function()
		local names = { }
		for _, hire in ipairs(roster.mercenaries) do
			assert.is_nil(names[hire.name], hire.name)
			names[hire.name] = true
		end
		...
		local hire = roster.mercenaries[1]
		local association = Import.association(source.account, source.realm, source.league, hire)
		assert.is_true(Import.matchesAssociation(association, source.account, source.realm, source.league, hire))
		assert.is_false(Import.matchesAssociation(association, source.account, source.realm, source.league, hireA))
		assert.is_false(Import.matchesAssociation(nil, source.account, source.realm, source.league, hire))
		local original = import(1)
		assert.equals(association, original.importAssociation)
		assert.equals(original.id, import(1).id)
	end)

	it("binds a newly imported hire to the active loadout", function()
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		local first = import(1)
		assert.are.equal(first.id, configSet.mercenarySetId)
		local second = import(2)
		assert.are.equal(second.id, configSet.mercenarySetId)
		build:SyncLoadouts()
		assert.are.equal("Default", build.controls.buildLoadouts.list[build.controls.buildLoadouts.selIndex])
	end)

	it("does not rebind the active loadout when reimporting an existing hire", function()
		local first = import(1)
		local second = import(2)
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		configSet.mercenarySetId = first.id
		assert.are.equal(second.id, import(2).id)
		assert.are.equal(first.id, configSet.mercenarySetId)
	end)
end)
