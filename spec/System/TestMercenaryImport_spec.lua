describe("Mercenary API import", function()
	...
	it("hashes legacy reversible associations on load", function()
		...
		tab:Save(xml)
		tab:Load(xml)
		assert.equals(Import.legacyAssociation(uuid, source.realm, source.league, hire), tab.profile.importAssociation)
		assert.is_true(Import.matchesAssociation(tab.profile.importAssociation, uuid, source.realm, source.league, hire))
	...
	it("API association includes skill hashes because LeagueAccount has no unique Mercenary id", function()
		...
		assert.equals(Import.legacyAssociation(source.account, source.realm, source.league, hireA), Import.legacyAssociation(source.account, source.realm, source.league, hireB))
		local hire = roster.mercenaries[1]
		local legacy = Import.legacyAssociation(source.account, source.realm, source.league, hire)
		assert.is_true(Import.matchesAssociation(legacy, source.account, source.realm, source.league, hire))
		assert.not_equals(legacy, Import.association(source.account, source.realm, source.league, hire))
		local original = import(1)
		tab.profile.importAssociation = legacy
		assert.equals(original.id, import(1).id)
		assert.equals(Import.association(source.account, source.realm, source.league, hire), tab.profile.importAssociation)
	end)
end)
