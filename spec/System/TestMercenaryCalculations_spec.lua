describe("Permanent Mercenary calculations", function()
	...
	local function assertStaticMercenaryError(env, pattern)
		assert.is_nil(env.mercenary)
		assert.is_nil(env.mercenaryCalculationErrors)
		local errors = build.mercenaryTab:GetErrors()
		assert.matches(pattern, table.concat(errors, "\n"))
		local seen = { }
		for _, errorText in ipairs(errors) do
			assert.is_nil(seen[errorText], errorText)
			seen[errorText] = true
		end
	end
	...
	it("keeps Infamous Mercenary mods and support stat maps off player tables", function()
		build.data.ensureMercenaries()
		assert.is_not_nil(build.data.itemMods.Mercenary.MercenaryModTravelSkillCrit)
		assert.is_nil(build.data.itemMods.Item.MercenaryModTravelSkillCrit)
		assert.are_not.equal(build.data.skillStatMap, build.data.mercenarySupportStatMap)
		local nested
		for statId, playerMap in pairs(build.data.skillStatMap) do
			local mercMap = build.data.mercenarySupportStatMap[statId]
			if mercMap then
				assert.are_not.equal(playerMap, mercMap, statId)
				local playerMod, mercMod = playerMap[1], mercMap[1]
				if playerMod and mercMod and type(playerMod.value) == "table" and playerMod.value.mod then
					assert.are_not.equal(playerMod.value.mod, mercMod.value.mod, statId)
					nested = true
					break
				end
			end
	...
	it("caches Mercenary skill output separately from the player", function()
		configure("TrapsMinesShadow", "TrapsMinesShadowLightning", "LightningTrapMercenary")
		local env = calculate()
		assert.is_not_nil(env.mercenary)
		local playerUUID = cacheSkillUUID(env.player.mainSkill, env)
		local mercUUID = cacheSkillUUID(env.mercenary.mainSkill, env)
		local playerCache = GlobalCache.cachedData.MAIN[playerUUID]
		local mercCache = GlobalCache.cachedData.MAIN[mercUUID]
		assert.are_not.equal(playerUUID, mercUUID)
		assert.are_not.equal(playerCache, mercCache)
		assert.are.equal(env.player.mainSkill, playerCache.ActiveSkill)
		assert.are.equal(env.mercenary.mainSkill, mercCache.ActiveSkill)
		assert.are.equal(env.mercenary.output.TotalDPS, mercCache.TotalDPS)
	end)
	...
	it("hires only with Noble Blood on Luminary", function()
		...
		assert.is_nil(unhired.player.enemySourceDB)
	...
	it("keeps independent loadouts and persists XML", function()
		...
		assert.is_true(build.calcsTab.input.legacyShowMinion)
		env = calculate()
		assert.is_nil(env.minion)
		assert.are.equal(env.player, build.calcsTab:GetDisplayActor(env))
		local savedCalcs = { }
		build.calcsTab:Save(savedCalcs)
		build.calcsTab:ResetUndo()
		build.calcsTab:SetCalculationActor("PLAYER_MINION")
		build.calcsTab:AddUndoState()
		assert.is_nil(build.calcsTab.input.legacyShowMinion)
		build.calcsTab:Undo()
		assert.is_true(build.calcsTab.input.legacyShowMinion)
		assert.are.equal(env.player, build.calcsTab:GetDisplayActor(env))
		build.calcsTab:Load(savedCalcs)
		assert.is_true(build.calcsTab.input.legacyShowMinion)
		assert.are.equal("PLAYER_MINION", build.calcsTab.input.actor)
		assert.are.equal(env.player, build.calcsTab:GetDisplayActor(env))
		build.calcsTab:SetCalculationActor("PLAYER_MINION")
		assert.is_nil(build.calcsTab.input.legacyShowMinion)
		assert.is_nil(build.calcsTab:GetDisplayActor(env))
		env = calculate()
		assert.is_nil(env.minion)
		assert.is_nil(build.calcsTab:GetDisplayActor(env))
...
Precise Technique
	...
	it("calculates extracted skill DPS, parts, exerts, and on-hit curses", function()
		...
		assert.are.equal(build.data.nonDamagingAilment.Shock.max, env.mercenary.output.MaximumShock)

		allocate("Legendary Rings")
		local taming = new("Item"):Item([[Rarity: UNIQUE
The Taming
Prismatic Ring
Implicits: 1
...
35% increased Elemental Damage with Hits and Ailments for each type of Elemental Ailment on Enemy]])
		build.itemsTab:AddItem(taming, true)
		equipmentSlot("Ring 1").selItemId = taming.id
		env = calculate(83)
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		assert.are.equal(build.data.nonDamagingAilment.Chill.max, env.mercenary.output.MaximumChill)
		assert.are.equal(build.data.nonDamagingAilment.Shock.max, env.mercenary.output.MaximumShock)
		...
		equipmentSlot("Ring 1").selItemId = shockCapRing.id
		env = calculate(83)
...
Adds 500 to 500 Physical Damage]])
	...
	it("honours incoming ally aura restrictions and stronger imported auras on a Mercenary", function()
		...
		equipmentSlot("Weapon 1").selItemId = sword.id
		local baseline = assert(calculate().mercenary).output.Evasion
		build.skillsTab:PasteSocketGroup("Grace 20/0  1")
		local env = calculate()
		assert.is_true(env.mercenary.modDB:Flag(nil, "AlliesAurasCannotAffectSelf"))
		...
		build.skillsTab:PasteSocketGroup("Grace 20/0  1")
		calculate()
		local exported = assert(build.partyTab.buffExports.Aura.Grace)
		local localOnly = calculate().mercenary.output.Evasion
		build.partyTab.actor.Aura = { Aura = { Grace = { effectMult = 200, modList = exported.modList } } }
		local withLocal = calculate().mercenary.output.Evasion
		for _, group in ipairs(build.skillsTab.socketGroupList) do
			for _, gem in ipairs(group.gemList) do gem.enabled = false end
		end
		local importedOnly = calculate().mercenary.output.Evasion
		...
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.actors.mercenary.customModsList[1].text = "100% increased Effect of Buffs on you"
		calculate()
		exported = assert(build.partyTab.buffExports.Aura.Grace)
		local scaled = calculate()
		assert.are.equal(100, scaled.mercenary.modDB:Sum("INC", nil, "BuffEffectOnSelf"))
		localOnly = scaled.mercenary.output.Evasion
		-- 150% imported is weaker than local if recipient self-effect is ignored
		-- (1.5 vs 2.0), but stronger once both sides include it (2.5 vs 2.0).
		build.partyTab.actor.Aura = { Aura = { Grace = { effectMult = 150, modList = exported.modList } } }
		withLocal = calculate().mercenary.output.Evasion
		for _, group in ipairs(build.skillsTab.socketGroupList) do
			for _, gem in ipairs(group.gemList) do gem.enabled = false end
		end
	...
	it("uses a stronger imported aura instead of a Mercenary's own weaker aura", function()
		...
		build.skillsTab:PasteSocketGroup("Zealotry 20/0  1")
		calculate()
		local exported = assert(build.partyTab.buffExports.Aura.Zealotry)
		for _, group in ipairs(build.skillsTab.socketGroupList) do
			for _, gem in ipairs(group.gemList) do gem.enabled = false end
		end
		local spellCfg = { flags = ModFlag.Spell }
		local localOnly = calculate().mercenary.modDB:More(spellCfg, "Damage")
		build.partyTab.actor.Aura = { Aura = { Zealotry = { effectMult = 200, modList = exported.modList } } }
		local withLocal = calculate().mercenary.modDB:More(spellCfg, "Damage")
		build.mercenaryTab.profile.skills[2].enabled = false
		build.mercenaryTab:Changed()
		local importedOnly = calculate().mercenary.modDB:More(spellCfg, "Damage")
		assert.is_true(importedOnly > localOnly)
	...
	it("applies recipient self-effect when a stronger imported aura replaces a Mercenary aura on the player", function()
		...
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "100% increased Effect of Buffs on you"
		local spellCfg = { flags = ModFlag.Spell }
		local localOnly = calculate().player.modDB:More(spellCfg, "Damage")
		assert.are.equal(100, calculate().player.modDB:Sum("INC", nil, "BuffEffectOnSelf"))
		-- 150% imported is weaker than local if recipient self-effect is ignored
		-- (1.5 vs 2.0), but stronger once both sides include it (2.5 vs 2.0).
		build.partyTab.actor.Aura = { Aura = { Zealotry = { effectMult = 150, modList = exported.modList } } }
		local withLocal = calculate().player.modDB:More(spellCfg, "Damage")
		build.mercenaryTab.profile.skills[1].enabled = false
		build.mercenaryTab:Changed()
	...
	it("applies recipient self-effect when a stronger imported aura replaces the player's own weaker aura", function()
		...
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "100% increased Effect of Buffs on you"
		local localOnly = calculate().player.output.Evasion
		assert.are.equal(100, calculate().player.modDB:Sum("INC", nil, "BuffEffectOnSelf"))
		-- 150% imported is weaker than local if recipient self-effect is ignored
		-- (1.5 vs 2.0), but stronger once both sides include it (2.5 vs 2.0).
		build.partyTab.actor.Aura = { Aura = { Grace = { effectMult = 150, modList = exported.modList } } }
		local withLocal = calculate().player.output.Evasion
		for _, group in ipairs(build.skillsTab.socketGroupList) do
			for _, gem in ipairs(group.gemList) do gem.enabled = false end
		end
	...
	it("arbitrates imported Vaal auras and marks the underlying aura condition", function()
		...
		calculate()
		local exported = assert(build.partyTab.buffExports.Aura["Vaal Grace"])
		local env = calculate()
		local localOnly = env.mercenary.modDB:Sum("BASE", nil, "EvadeChance")
		assert.is_true(env.mercenary.modDB.conditions.AffectedByVaalGrace)
		assert.is_true(env.mercenary.modDB.conditions.AffectedByGrace)
		build.partyTab.actor.Aura = { Aura = { Vaal = { ["Vaal Grace"] = { effectMult = 200, modList = exported.modList } } } }
		local withLocal = calculate().mercenary.modDB:Sum("BASE", nil, "EvadeChance")
		for _, group in ipairs(build.skillsTab.socketGroupList) do
			for _, gem in ipairs(group.gemList) do gem.enabled = false end
		end
		env = calculate()
		local importedOnly = env.mercenary.modDB:Sum("BASE", nil, "EvadeChance")
		assert.is_true(importedOnly > localOnly)
		assert.are.equal(importedOnly, withLocal)
		assert.is_true(env.mercenary.modDB.conditions.AffectedByVaalGrace)
		assert.is_true(env.mercenary.modDB.conditions.AffectedByGrace)
	...
	it("applies player Quicksilver flasks to a Mercenary with QuickSilverAppliesToAllies", function()
		configure("TrapsMinesShadow", "TrapsMinesShadowLightning", "LightningTrapMercenary")
		local silver = new("Item"):Item("Rarity: Normal\nQuicksilver Flask")
		build.itemsTab:AddItem(silver, true)
		build.itemsTab.slots["Flask 1"].selItemId = silver.id
		build.itemsTab.slots["Flask 1"].active = true
		build.itemsTab.activeItemSet["Flask 1"].selItemId = silver.id
		build.itemsTab.activeItemSet["Flask 1"].active = true
		local baseline = calculate()
		assert.is_true(baseline.player.modDB.conditions.UsingQuicksilverFlask)
		assert.is_nil(baseline.mercenary.modDB.conditions.UsingQuicksilverFlask)
		local baseMove = baseline.mercenary.modDB:Sum("INC", nil, "MovementSpeed")
		build.configTab.input.customMods = "Quicksilver Flasks you Use also apply to nearby Allies"
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "Quicksilver Flasks you Use also apply to nearby Allies"
		build.configTab:BuildModList()
		local env = calculate()
		assert.is_true(env.mercenary.modDB.conditions.UsingQuicksilverFlask)
		assert.is_true(env.mercenary.modDB.conditions.UsingFlask)
		assert.is_true((env.mercenary.appliedFlasks or { })[silver])
		assert.is_true(env.mercenary.modDB:Sum("INC", nil, "MovementSpeed") > baseMove)

		configure("AurasMinionsTemplar", "AurasMinionsTemplarStaff", "HeraldOfPurityMercenary")
		env = calculate()
		assert.is_table(env.mercenaryMinion)
		assert.is_true(env.mercenaryMinion.modDB.conditions.UsingQuicksilverFlask)
...
+100 to maximum Life]])
		...
		itemsTab = build.itemsTab
		mercSet = assert(build.mercenaryTab:GetItemSet(true))
		local unique = new("Item"):Item("Rarity: Unique\nGoldrim\nLeather Cap\n+30% to all Elemental Resistances")
		itemsTab:AddItem(unique, true)
		calculate()
		assert(itemsTab:SetViewItemSet(mercSet.id, "MERCENARY"))
		local calc = build.calcsTab:GetMiscCalculator()
		local uniqueError = "Helmet: requires a modifier allowing your Mercenary to equip Unique Helmets"
		local preview = calc(itemsTab:ItemCalculationOverride("Helmet", unique))
		mercSet.Helmet.selItemId = unique.id
		local equipped = calc({ comparisonActor = "MERCENARY" })
		local removed = calc(itemsTab:ItemCalculationOverride("Helmet", nil))
		mercSet.Helmet.selItemId = 0
		local cleared = calc({ comparisonActor = "MERCENARY" })
		assert.are.equal(uniqueError, equipped.ActorUnavailableMessage)
		assert.is_nil(cleared.ActorUnavailableMessage)
		assert.are.equal(equipped.ActorUnavailableMessage, preview.ActorUnavailableMessage)
		...
		itemsTab = build.itemsTab
		mercSet = assert(build.mercenaryTab:GetItemSet(true))
		local mainDagger = new("Item"):Item("Rarity: Normal\nGlass Shank")
		local offDagger = new("Item"):Item("Rarity: Normal\nGlass Shank")
		local previewWand = new("Item"):Item("Rarity: Normal\nDriftwood Wand")
		itemsTab:AddItem(mainDagger, true)
		itemsTab:AddItem(offDagger, true)
		itemsTab:AddItem(previewWand, true)
		mercSet["Weapon 1"].selItemId = mainDagger.id
		mercSet["Weapon 2"].selItemId = offDagger.id
		calculate()
		assert(itemsTab:SetViewItemSet(mercSet.id, "MERCENARY"))
		calc = build.calcsTab:GetMiscCalculator()
		local dualWield = calc({ comparisonActor = "MERCENARY" })
		local wandPreview = calc(itemsTab:ItemCalculationOverride("Weapon 1", previewWand))
...
+100 to maximum Life]])
		...
		guardianGem = assert(guardianGem)
		guardianGem.skillMinionItemSet = selectedGuardianSet.id
		guardianGem.skillMinionItemSetCalcs = selectedGuardianSet.id
	...
	it("applies Animate Guardian extra auras to the player and Mercenary when AG is not the main skill", function()
		configure("TrapsMinesShadow", "TrapsMinesShadowLightning", "LightningTrapMercenary")
		local itemsTab = build.itemsTab
		local guardianSet = itemsTab:NewItemSet()
		guardianSet.title = "Animate Guardian"
		...
		local leerCast = new("Item"):Item([[Rarity: Rare
Leer Cast
Festival Mask
You and nearby allies gain 50% increased Damage]])
		local kingmaker = new("Item"):Item([[Rarity: Rare
Kingmaker
Despot Axe
Nearby Allies have Culling Strike
Nearby Allies have +50% to Critical Strike Multiplier
Nearby Allies have +10 Fortification]])
		itemsTab:AddItem(leerCast, true)
		itemsTab:AddItem(kingmaker, true)
		guardianSet.Helmet.selItemId = leerCast.id
		guardianSet["Weapon 1"].selItemId = kingmaker.id

		build.skillsTab:PasteSocketGroup("Precision 20/0  1")
		build.skillsTab:PasteSocketGroup("Animate Guardian 20/0  1")
		build.mainSocketGroup = 1
		local guardianGroup = build.skillsTab.socketGroupList[#build.skillsTab.socketGroupList]
		local guardianGem
		...
		for _, skill in ipairs(env.player.activeSkillList) do
			if skill.minion and skill.minion.itemSet == guardianSet then
				guardian = skill.minion
				break
			end
		end
		assert.is_not_nil(guardian)
		assert.are.equal(leerCast, guardian.itemList.Helmet)
		assert.are.equal(kingmaker, guardian.itemList["Weapon 1"])

		local playerFort = env.player.modDB:Sum("BASE", nil, "MinimumFortification")
		local mercFort = env.mercenary.modDB:Sum("BASE", nil, "MinimumFortification")
		local playerCull = env.player.modDB:Max(nil, "CullPercent") or 0
		local mercCull = env.mercenary.modDB:Max(nil, "CullPercent") or 0
		local playerDamage = env.player.modDB:Sum("INC", nil, "Damage")
		local mercDamage = env.mercenary.modDB:Sum("INC", nil, "Damage")
		local playerCritMulti = env.player.modDB:Sum("BASE", nil, "CritMultiplier")
		local mercCritMulti = env.mercenary.modDB:Sum("BASE", nil, "CritMultiplier")
		assert.is_true(playerFort >= 10, "player should receive Kingmaker fortification")
		assert.is_true(mercFort >= 10, "Mercenary should receive Kingmaker fortification")
		assert.is_true(playerCull >= 10, "player should receive Kingmaker culling strike")
		assert.is_true(mercCull >= 10, "Mercenary should receive Kingmaker culling strike")
		assert.is_true(playerDamage >= 50, "player should receive Leer Cast extra aura")
		assert.is_true(mercDamage >= 50, "Mercenary should receive Leer Cast extra aura")
		assert.is_true(playerCritMulti >= 50, "player should receive Kingmaker crit multiplier")
		assert.is_true(mercCritMulti >= 50, "Mercenary should receive Kingmaker crit multiplier")

		guardianGem.enabled = false
		env = calculate()
		assert.is_true(env.player.modDB:Sum("BASE", nil, "MinimumFortification") < playerFort)
		assert.is_true(env.mercenary.modDB:Sum("BASE", nil, "MinimumFortification") < mercFort)
		assert.is_true((env.player.modDB:Max(nil, "CullPercent") or 0) < playerCull)
		assert.is_true((env.mercenary.modDB:Max(nil, "CullPercent") or 0) < mercCull)
		assert.is_true(env.player.modDB:Sum("INC", nil, "Damage") < playerDamage)
		assert.is_true(env.mercenary.modDB:Sum("INC", nil, "Damage") < mercDamage)
		assert.is_true(env.player.modDB:Sum("BASE", nil, "CritMultiplier") < playerCritMulti)
		assert.is_true(env.mercenary.modDB:Sum("BASE", nil, "CritMultiplier") < mercCritMulti)
	end)
...
Avatar of Fire
		...
		equipmentSlot("Amulet").selItemId = amulet.id
		build.skillsTab:PasteSocketGroup("Arc 20/0  1")
		local playerGroup = build.skillsTab.socketGroupList[#build.skillsTab.socketGroupList]
		playerGroup.includeInFullDPS = false
		local env = calculate()
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "; "))
	...
	it("uses stronger Mercenary EE exposure over an existing weaker exposure", function()
		...
		build.configTab:EnsureActorConfig(configSet)
		configSet.actors.mercenary.customModsList[1].text = elementalEquilibriumMod
		local before = calculate()
		assert.is_true(before.mercenary.enemySourceDB:GetCondition("HitByLightningDamage"))
		local beforeCold = before.enemyDB:Sum("BASE", nil, "ColdResist")
	...
	it("clears HasLightningExposure when EE later hides that exposure", function()
		...
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "Nearby Enemies have Lightning Exposure"
		local before = calculate()
		assert.is_table(before.mercenary, table.concat(before.mercenaryCalculationErrors or { }, "\n"))
		local beforeLightning = before.enemyDB:Sum("BASE", nil, "LightningResist")
		assert.is_true(before.enemyDB:GetCondition("HasLightningExposure"))
		configSet.actors.mercenary.customModsList[1].text = elementalEquilibriumMod
		local after = calculate()
		assert.is_true(after.mercenary.enemySourceDB:GetCondition("HitByLightningDamage"))
		assert.are.equal(beforeLightning + 10, after.enemyDB:Sum("BASE", nil, "LightningResist"))
		assert.is_not_true(after.enemyDB:GetCondition("HasLightningExposure"))
		assert.is_not_true(after.enemyDB:Flag(nil, "Condition:HasLightningExposure") or after.enemy.modDB.conditions.HasLightningExposure)
		assert.is_true(after.enemyDB:GetCondition("HasFireExposure") or after.enemyDB:Flag(nil, "Condition:HasFireExposure"))
		assert.is_true(after.enemyDB:GetCondition("HasColdExposure") or after.enemyDB:Flag(nil, "Condition:HasColdExposure"))
	end)

	it("previews supports for the selected non-main skill", function()
		configure("TrapsMinesShadow", "TrapsMinesShadowLightning", "LightningTrapMercenary")
		local tab = build.mercenaryTab
		tab.profile.skills[2] = { id = "ZealotryMercenary", enabled = true, supports = { } }
		tab.profile.mainSkillId = "ZealotryMercenary"
		tab.selectedSkillIndex = 1
		tab:Changed()
		calculate()
		local base, modified
		local original = build.AddStatComparesToTooltip
		build.AddStatComparesToTooltip = function(_, _, before, after)
			base, modified = before, after
		end
		local support = assert(build.data.mercenaries.supports.AddedLightningHigh)
		local ok, err = pcall(tab.AddSupportTooltip, tab, new("Tooltip"):Tooltip(), support, 1, true)
		build.AddStatComparesToTooltip = original
		assert.is_true(ok, err)
		assert.is_truthy(base and modified)
		local hoveredDelta = (modified.CombinedDPS or 0) - (base.CombinedDPS or 0)
		tab.profile.mainSkillId = "LightningTrapMercenary"
		local expectedBefore = calculate().mercenary.output.CombinedDPS
		tab.profile.skills[1].supports = { { id = "AddedLightningHigh", tier = 3 } }
		local expectedAfter = calculate().mercenary.output.CombinedDPS
		assert.are.near(expectedAfter - expectedBefore, hoveredDelta, 1e-8)
		assert.is_true(hoveredDelta > 0)
	end)
...
+40 to maximum Life]])
	...
	it("keeps spectre skill-id configuration visible for the owning actor", function()
		local spectreId = "Metadata/Monsters/DemonModular/DemonModularBladeVortex"
		assert.is_truthy(isValueInArray(build.data.minions[spectreId].skillList, "DemonModularBladeVortexSpectre"))
		build.spectreList = { spectreId }
		build.skillsTab:PasteSocketGroup("Raise Spectre 20/0  1")
		local env = calculate()
		assert.is_true(env.skillsUsed.DemonModularBladeVortexSpectre)
		local option
		for _, candidate in ipairs(configOptions) do
			if candidate.var == "raiseSpectreBladeVortexBladeCount" then option = candidate break end
		end
		assert.is_true(configVisibility.isRelevantForBuild(assert(option), build, "player"))
	...
	it("applies player spectre AllyModifier buffs to Mercenaries", function()
		...
		configure("MeleeAOEMarauder", "MeleeAOEMarauderFireSlam", "FissureSlamMercenary")
		local env = calculate()
		assert.is_not_true(env.player.modDB:Flag(nil, "Condition:Onslaught"))
		assert.is_not_true(env.mercenary.modDB:Flag(nil, "Condition:Onslaught"))

		build.spectreList = { spectreId }
		build.skillsTab:PasteSocketGroup("Raise Spectre 20/0  1")
		env = calculate()
		assert.is_not_nil(env.minion)
		assert.are.equal(spectreId, env.minion.type)
		assert.is_true(env.player.modDB:HasMod("FLAG", nil, "Condition:Onslaught"))
		assert.is_true(env.mercenary.modDB:HasMod("FLAG", nil, "Condition:Onslaught"))
		assert.is_true(env.player.modDB:Flag(nil, "Condition:Onslaught"))
		assert.is_true(env.mercenary.modDB:Flag(nil, "Condition:Onslaught"))
		if env.mercenaryMinion then
			assert.is_true(env.mercenaryMinion.modDB:HasMod("FLAG", nil, "Condition:Onslaught"))
		end
	end)

	it("applies monster Soul Eater damage reduction to Mercenaries", function()
		allocate("Legendary Gloves")
		configure("ElementalWitch", "ElementalWitchLightning", "ArcMercenary")
		local gloves = new("Item"):Item([[Rarity: UNIQUE
Soul Ascension
Carnal Mitts
Implicits: 0
...
Eat a Soul when you Hit a Rare or Unique Enemy, no more than once every 0.5 seconds]])
		build.itemsTab:AddItem(gloves, true)
		equipmentSlot("Gloves").selItemId = gloves.id
		local config = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(config)
		config.actors.mercenary.input.multiplierSoulEater = 0
		local env = calculate()
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		local before = env.mercenary.output.FireMaximumHitTaken
		local speed = env.mercenary.output.Speed
		local playerBefore = env.player.output.FireMaximumHitTaken
		config.actors.mercenary.input.multiplierSoulEater = 20
		env = calculate()
		assert.is_true(env.mercenary.output.Speed > speed)
		assert.is_true(env.mercenary.output.FireMaximumHitTaken > before)
		assert.are.equal(playerBefore, env.player.output.FireMaximumHitTaken)
	end)

	it("scales Mercenary quiver bonuses from Widowhail", function()
		allocate("Legendary Arms")
		configure("EleBowRanger", "EleBowRangerClones", "MirrorArrowMercenary")
		local function add(raw)
			local value = new("Item"):Item(raw)
			build.itemsTab:AddItem(value, true)
			return value
		end
		equipmentSlot("Weapon 1").selItemId = add("Rarity: Normal\nCrude Bow").id
		equipmentSlot("Weapon 2").selItemId = add("Rarity: Rare\nReview Quiver\nTwo-Point Arrow Quiver\n+100 to maximum Life").id
		local before = assert(calculate().mercenary).output.Life
		equipmentSlot("Weapon 1").selItemId = add("Rarity: Unique\nWidowhail\nCrude Bow\n250% increased bonuses gained from Equipped Quiver").id
		local env = calculate()
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		assert.are.equal(250, env.mercenary.modDB:Sum("INC", nil, "EffectOfBonusesFromQuiver"))
		assert.are.near(3.5, env.mercenary.modDB:Sum("BASE", nil, "WidowHailMultiplier"), 1e-9)
		assert.is_true(env.mercenary.output.Life > before)
	end)

	it("keeps the Mercenary in calculations when disabling the displayed skill", function()
		configure("ElementalWitch", "ElementalWitchLightning", "ArcMercenary")
		local tab = build.mercenaryTab
		assert(tab:SetSkill(2, "SparkMercenary"))
		assert.is_table(calculate().mercenary)
		tab.selectedSkillIndex = 1
		tab.controls.skillEnabled.changeFunc(false)
		local env = calculate()
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		assert.are.equal("SparkMercenary", tab.profile.mainSkillId)
		assert.are.equal("SparkMercenary", env.mercenary.mainSkill.mercenarySkill.id)
	end)

	it("applies Mercenary socket occupancy to socket-property bonuses and empty-socket counts", function()
		allocate("Legendary Rings")
		allocate("Legendary Amulets")
		configure("ElementalWitch", "ElementalWitchLightning", "ArcMercenary")
		local emptyRing = new("Item"):Item([[Rarity: UNIQUE
Malachai's Artifice
Unset Ring
Sockets: W
Implicits: 1
...
All Sockets are White]])
		build.itemsTab:AddItem(emptyRing, true)
		equipmentSlot("Ring 1").selItemId = emptyRing.id
		local env = calculate()
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		local before = env.mercenary.output.FireResistTotal
		local occupiedRing = new("Item"):Item([[Rarity: UNIQUE
Malachai's Artifice
Unset Ring
Sockets: W
...
+100% to Fire Resistance when Socketed with a Red Gem
All Sockets are White]])
		build.itemsTab:AddItem(occupiedRing, true)
		equipmentSlot("Ring 1").selItemId = occupiedRing.id
		env = calculate()
		assert.are.equal(before + 100, env.mercenary.output.FireResistTotal)
		assert.are.equal(0, env.mercenary.modDB.multipliers.EmptyWhiteSocketsInAnySlot or 0)
...
Selected Variant: 2
Implicits: 1
{range:1}(12-20)% increased Rarity of Items found
...
{variant:1}+18% to all Elemental Resistances for each Empty White Socket on any Equipped Item
{variant:2}+12% to all Elemental Resistances for each Empty White Socket on any Equipped Item]])
		build.itemsTab:AddItem(amulet, true)
		equipmentSlot("Amulet").selItemId = amulet.id
		env = calculate()
		local occupiedWithAmulet = env.mercenary.output.FireResistTotal
		assert.are.equal(0, env.mercenary.modDB.multipliers.EmptyWhiteSocketsInAnySlot or 0)
		equipmentSlot("Ring 1").selItemId = emptyRing.id
		env = calculate()
		assert.are.equal(1, env.mercenary.modDB.multipliers.EmptyWhiteSocketsInAnySlot)
		assert.are.equal(occupiedWithAmulet - 100 + 12, env.mercenary.output.FireResistTotal)
	end)

	it("scores support sort previews with the same dense list as SetSupport", function()
		configure("MeleeAOEMarauder", "MeleeAOEMarauderFireSlam", "FissureSlamMercenary")
		local weapon = new("Item"):Item("Rarity: Normal\nStone Axe")
		build.itemsTab:AddItem(weapon, true)
		equipmentSlot("Weapon 1").selItemId = weapon.id
		local tab = build.mercenaryTab
		tab:EnsureData()
		local skill = tab.profile.skills[1]
		for _, id in ipairs({ "FistOfWarHigh", "AddedFireHigh" }) do
			table.insert(skill.supports, { id = id, tier = assert(tab.data.supports[id]).variant })
		end
		tab:Changed()
		tab:RefreshControls()
		...
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		local calc = build.calcsTab:GetMiscCalculator()
		local first = table.remove(skill.supports, 1)
		local removed = calc({ comparisonActor = "MERCENARY" })
		assert(not removed.ActorUnavailableMessage, removed.ActorUnavailableMessage)
		table.insert(skill.supports, 1, first)
		tab:QueueSupportSort(1)
		while tab.supportSortCoroutine do tab:ProcessSupportSort() end
		local removeEntry
		for _, entry in ipairs(assert(tab.supportSortCache[1]).list) do
			if not entry.id then removeEntry = entry end
		end
		assert.are.near(removed.CombinedDPS, assert(removeEntry).dps, 1e-8)

		skill.supports = { }
		tab:Changed()
		tab:RefreshControls()
		tab:QueueSupportSort(3)
		while tab.supportSortCoroutine do tab:ProcessSupportSort() end
		local addEntry
		for _, entry in ipairs(assert(tab.supportSortCache[3]).list) do
			if entry.id == "AddedFireHigh" then addEntry = entry end
		end
		assert(addEntry)
		assert(tab:SetSupport(3, addEntry.id))
		local added = calc({ comparisonActor = "MERCENARY" })
		assert(not added.ActorUnavailableMessage, added.ActorUnavailableMessage)
		assert.are.near(added.CombinedDPS, addEntry.dps, 1e-8)
	end)
	...
	it("preserves imported party Destructive Link crit chance", function()
		...
		local party = build.partyTab.actor
		build.partyTab:ParseBuffs(party.modDB, "MainHand.CritChance=25", "PartyMemberStats", party.output)
		local link = modLib.createMod("MainHandCritIsEqualToParent", "FLAG", true, "Skill:DestructiveLink", 0, 0, { type = "GlobalEffect", effectType = "Link" })
		build.partyTab:ParseBuffs(party.Link, "Destructive Link\n100\n"..modLib.formatSourceMod(link), "Link")
		build.configTab:BuildModList()
		local env = calcs.initEnv(build, "CALCULATOR")
		calcs.perform(env, true)
		assert.are.equal(25, party.output.MainHand.CritChance)
		assert.are.equal(25, calcs.actorMainHandSheetCritChance(party))
		assert.are.near(25, env.player.output.PreEffectiveCritChance, 1e-8)
	end)

	it("rejects Mercenary item auras when the player equips Ichimonji", function()
		configureSkill("FireballMercenary")
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "Your Mercenary can equip Unique Helmets"
		build.skillsTab:PasteSocketGroup("Arc 20/0  1")
		local sword = new("Item"):Item("Rarity: Unique\nIchimonji\nCorsair Sword\nAllies' Aura Buffs do not affect you\nYour Aura Buffs do not affect allies")
		build.itemsTab:AddItem(sword, true)
		build.itemsTab.activeItemSet["Weapon 1"].selItemId = sword.id
		local hat = new("Item"):Item("Rarity: Unique\nLeer Cast\nFestival Mask\nYou and nearby allies gain 50% increased Damage\n30% reduced Damage")
		build.itemsTab:AddItem(hat, true)
		local without = calculate()
		assert.is_table(without.mercenary, table.concat(without.mercenaryCalculationErrors or { }, "\n"))
		equipmentSlot("Helmet").selItemId = hat.id
		local with = calculate()
		assert.is_table(with.mercenary, table.concat(with.mercenaryCalculationErrors or { }, "\n"))
		assert.are.equal(hat, with.mercenary.itemList.Helmet)
		assert.is_true(with.modDB:Flag(nil, "AlliesAurasCannotAffectSelf"))
		assert.are.near(without.player.output.CombinedDPS, with.player.output.CombinedDPS, 1e-8)
	end)

	it("applies Mercenary Vortex's guaranteed chill", function()
		configureSkill("VortexMercenary")
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "100% increased Damage against Chilled Enemies"
		build.skillsTab:PasteSocketGroup("Arc 20/0  1")
		build.configTab:BuildModList()
		local env = calcs.initEnv(build, "CALCULATOR")
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
	...
	it("applies mercenary shock above the player's ailment cap", function()
		...
		build.configTab.input.conditionShockEffect = 70
		local env = calculate(83)
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		assert.are.equal(build.data.nonDamagingAilment.Shock.max, env.player.output.MaximumShock)
		assert.are.equal(build.data.nonDamagingAilment.Shock.max + 40, env.mercenary.output.MaximumShock)
	...
	it("keeps Mercenary Assassin's Mark on a Hexproof enemy", function()
		...
		equipmentSlot("Weapon 1").selItemId = bow.id
		equipmentSlot("Weapon 2").selItemId = quiver.id
		build.configTab:BuildModList()
		local env = calcs.initEnv(build, "CALCULATOR")
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		env.enemyDB:NewMod("Hexproof", "FLAG", true, "Review")
		calcs.perform(env, true)
		assert.is_true(env.enemyDB:GetCondition("Marked"))
		assert.is_true(#env.curseSlots > 0)
	end)

	it("sorts supports for the selected Mercenary skill when another skill is main", function()
		configure("TrapsMinesShadow", "TrapsMinesShadowLightning", "LightningTrapMercenary")
		local tab = build.mercenaryTab
		tab.profile.skills[2] = { id = "ZealotryMercenary", enabled = true, supports = { } }
		tab.profile.mainSkillId = "ZealotryMercenary"
		...
		tab:Changed()
		tab:RefreshControls()
		tab.sortGemsByDPS, tab.sortGemsByDPSField = true, "CombinedDPS"
		tab:QueueSupportSort(1)
		while tab.supportSortCoroutine do tab:ProcessSupportSort() end
		local scored
		for _, row in ipairs(assert(tab.supportSortCache[1]).list) do
			if row.id == "AddedLightningHigh" then scored = row.dps end
		end
		assert(scored)
		assert.is_true(scored > 0)
		tab.profile.mainSkillId = "LightningTrapMercenary"
	...
	it("uses Average Hit when sorting Mercenary minion supports", function()
		...
		tab:Changed()
		tab:RefreshControls()
		tab.sortGemsByDPS, tab.sortGemsByDPSField = true, "AverageDamage"
		tab:QueueSupportSort(1)
		while tab.supportSortCoroutine do tab:ProcessSupportSort() end
		local selectedRow
		for _, row in ipairs(assert(tab.supportSortCache[1]).list) do
			if row.id == "AddedColdHigh" then selectedRow = row break end
		end
		assert(selectedRow)
		tab.profile.skills[1].supports = { { id = selectedRow.id, tier = selectedRow.tier } }
		tab:Changed()
		local env = calculate()
		assert.is_table(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		local output = assert(env.mercenary.output.Minion)
		assert.are.near(output.AverageDamage, selectedRow.dps, 1e-8)
	...
	it("gives mercenaries player combat inherents, curse slots, skill setup, and aura accounting", function()
		...
		local env = calculate()
		local constants = build.data.characterConstants
		assert.are.equal(1, env.mercenary.modDB:Sum("BASE", nil, "EnemyCurseLimit"))
		assert.are.equal(constants["base_presence_radius"], env.mercenary.modDB:Sum("BASE", nil, "PresenceRadius"))
		local bleedMore
		for _, mod in ipairs(env.mercenary.modDB.mods.Damage or { }) do
			if mod.type == "MORE" and mod.source == "Base" and mod.value == 200 and mod.keywordFlags == KeywordFlag.Bleed then
				bleedMore = mod
			end
		end
		assert.is_not_nil(bleedMore)

		local curseHelmet = new("Item"):Item("Rarity: Rare\nCurse Limit\nIron Hat\nYou can apply an additional Curse")
		build.itemsTab:AddItem(curseHelmet, true)
		equipmentSlot("Helmet").selItemId = curseHelmet.id
		env = calculate()
		...
		local configSet = build.configTab.configSets[build.configTab.activeConfigSetId]
		build.configTab:EnsureActorConfig(configSet)
		configSet.customModsList[1].text = "You can apply an additional Curse"
		build.skillsTab:PasteSocketGroup("Despair 20/0  1")
		env = calculate()
		local slotted, names = { }, { }
		for _, slot in ipairs(env.curseSlots) do
			slotted[slot.name] = true
			table.insert(names, slot.name)
		end
		assert.is_true(slotted.Despair, table.concat(names, ", "))
		...
		build.mercenaryTab.profile.skills = {
			...
			{ id = "FlammabilityBlasphemyMercenary", enabled = true, supports = { } },
		}
		build.mercenaryTab.profile.mainSkillId = "AlchemistsMarkMercenary"
		build.mercenaryTab:Changed()
		local bow = new("Item"):Item("Rarity: Normal\nCrude Bow")
		local quiver = new("Item"):Item("Rarity: Normal\nSerrated Arrow Quiver")
		build.itemsTab:AddItem(bow, true)
		...
		env = calculate()
		local mercCurses, curseNames = 0, { }
		for _, slot in ipairs(env.curseSlots) do
			table.insert(curseNames, slot.name..(slot.sourceActor == env.mercenary and "@merc" or "@other"))
			if slot.sourceActor == env.mercenary then
				mercCurses = mercCurses + 1
			end
		end
		assert.are.equal(1, env.mercenary.modDB:Sum("BASE", nil, "EnemyCurseLimit"))
		...
		local maul = new("Item"):Item("Rarity: Normal\nDriftwood Maul")
		build.itemsTab:AddItem(maul, true)
		equipmentSlot("Weapon 1").selItemId = maul.id
		build.configTab.input.conditionEnemyIgnited = true
		env = calculate()
		local combustion
		for _, value in ipairs(env.enemyDB:Tabulate("BASE", nil, "FireResist")) do
			if value.mod.source == "Combustion" then combustion = value.value end
		end
		...
		local extraHelmet = {
			id = 9110, name = "Extra Aura Helm", type = "Helmet", base = { type = "Helmet" }, rarity = "RARE",
			requirements = { str = 1 }, grantedSkills = { },
			modList = {
				modLib.createMod("ExtraAuraEffect", "LIST", { mod = extraAura }),
				modLib.createMod("ExtraAuraEffect", "LIST", { mod = copyTable(extraAura, true) }),
			},
		}
		build.itemsTab.items[extraHelmet.id] = extraHelmet
		equipmentSlot("Helmet").selItemId = extraHelmet.id
		env = calculate()
		local extraAuraDamage = 0
		for _, value in ipairs(env.mercenary.modDB:Tabulate("INC", nil, "Damage")) do
			if value.mod.source == "TestExtraAura" then extraAuraDamage = extraAuraDamage + value.value end
		end
		local auraEffect = calcLib.mod(env.mercenary.mainSkill.skillModList, env.mercenary.mainSkill.skillCfg, "AuraEffect", "BuffEffect", "BuffEffectOnSelf", "AuraEffectOnSelf")
		...
		build.itemsTab:AddItem(shaperHelmet, true)
		build.itemsTab:AddItem(ironRing, true)
		equipmentSlot("Helmet").selItemId = shaperHelmet.id
		equipmentSlot("Ring 1").selItemId = ironRing.id
		env = calculate()
		assert.are.equal(1, env.mercenary.modDB.multipliers.ShaperItem)
		assert.are.equal(1, env.mercenary.modDB.multipliers.HelmetItem)
		assert.are.equal(1, env.mercenary.modDB.multipliers.RingItem)
		assert.are.equal(1, env.mercenary.modDB.multipliers.IronRingEquipped)

		local wings = new("Item"):Item([[Rarity: Rare
Dual Test
Stone Axe
Implicits: 0
Counts as Dual Wielding]])
		build.itemsTab:AddItem(wings, true)
		equipmentSlot("Helmet").selItemId = 0
		equipmentSlot("Ring 1").selItemId = 0
		equipmentSlot("Weapon 1").selItemId = wings.id
		equipmentSlot("Weapon 2").selItemId = 0
		env = calculate()
		assert.is_not_nil(env.mercenary, table.concat(env.mercenaryCalculationErrors or { }, "\n"))
		assert.is_true(env.mercenary.weaponData1.countsAsDualWielding)
		assert.is_truthy(env.mercenary.weaponData2.type)
		assert.is_true(env.mercenary.modDB.conditions.DualWielding)
		local dualWieldSpeed
		for _, mod in ipairs(env.mercenary.modDB.mods.Speed or { }) do
			if mod.type == "MORE" and mod.source == "Base" and mod.value == constants["dual_wield_inherent_attack_speed_+%_final"] then
				dualWieldSpeed = mod
			end
		end
		...
		build.itemsTab.activeItemSet["Flask 1"].active = true
		local ceinture = new("Item"):Item("Rarity: Unique\nCeinture of Benevolence\nCloth Belt\nNon-Unique Utility Flasks you Use apply to Linked Targets")
		build.itemsTab:AddItem(ceinture, true)
		build.itemsTab.slots.Belt.selItemId = ceinture.id
		build.itemsTab.activeItemSet.Belt.selItemId = ceinture.id
		env = calculate()
		assert.is_true(env.mercenary.modDB.conditions.UsingGraniteFlask)
		local hasBloodMagic
		for _, name in ipairs(env.mercenary.modDB:List(nil, "Keystone")) do
			if name == "Blood Magic" then hasBloodMagic = true end
		end
		...
		local enemyMultHelmet = {
			...
			requirements = { str = 1 }, grantedSkills = { },
			modList = {
				modLib.createMod("Damage", "INC", 20, "Merc Enemy Totem Mult", { type = "Multiplier", var = "TotemsSummoned", actor = "enemy" }),
			},
		}
		build.itemsTab.items[enemyMultHelmet.id] = enemyMultHelmet
		equipmentSlot("Helmet").selItemId = enemyMultHelmet.id
		env = calculate()
		local totemsConfig = { var = "TotemsSummoned", ifStat = "TotemsSummoned" }
		assert.is_nil(configVisibility.usedForVar(env, "enemyMultipliersUsed", totemsConfig, "player").TotemsSummoned)
		...
		local usageEnv = {
			actorUsage = {
				player = { enemyMultipliers = { TotemsSummoned = true } },
			},
			enemyMultipliersUsed = { TotemsSummoned = true },
		}
		local actorVar = { var = "TotemsSummoned", resolvedScope = "actor" }
		assert.is_nil(configVisibility.usedForVar(usageEnv, "enemyMultipliersUsed", actorVar, "mercenary").TotemsSummoned)
		assert.is_not_nil(configVisibility.usedForVar(usageEnv, "enemyMultipliersUsed", actorVar, "player").TotemsSummoned)
	end)
