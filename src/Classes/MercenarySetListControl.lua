...
---@class MercenarySetListControl: ListControl
...
function MercenarySetListClass:RenameSet(set, addOnName)
	...
	controls.save = new("ButtonControl"):ButtonControl(nil, {-45, 70, 80, 20}, "Save", function()
		...
		if addOnName then
			...
			local source = set.itemSetId and itemsTab.itemSets[set.itemSetId]
			if source then
				local cloned = copyTable(source)
				cloned.id = 1
				while itemsTab.itemSets[cloned.id] do
					cloned.id = cloned.id + 1
				end
				cloned.title = set.title
				itemsTab.itemSets[cloned.id] = cloned
				t_insert(itemsTab.itemSetOrderList, cloned.id)
				set.itemSetId = cloned.id
		...
		self.mercenaryTab:AddUndoState()
		self.mercenaryTab.build:SyncLoadouts()
		main:ClosePopup()
	end)
...
function MercenarySetListClass:OnSelClick(_, setId, doubleClick)
	if doubleClick and setId ~= self.mercenaryTab.activeMercenarySetId then
		self.mercenaryTab:SetActiveMercenarySet(setId, false, true)
		self.mercenaryTab:AddUndoState()
	end
end
...
function MercenarySetListClass:OnSelDelete(index, setId)
	...
	if #self.list > 1 then
		main:OpenConfirmPopup("Delete Mercenary Loadout", "Are you sure you want to delete '"..(set.title or "Default").."'?", "Delete", function()
			self.mercenaryTab:ClearConfigBindings(setId)
			t_remove(self.list, index)
			self.mercenaryTab.mercenarySets[setId] = nil
			self.selIndex = nil
			self.selValue = nil
			if setId == self.mercenaryTab.activeMercenarySetId then
				self.mercenaryTab:SetActiveMercenarySet(self.list[m_max(1, index - 1)], false, false)
			else
				self.mercenaryTab:RefreshControls()
				self.mercenaryTab.build:SyncLoadouts()
			end
