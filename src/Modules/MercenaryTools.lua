...
MercenaryTools.MONSTER_SPEED_DAMAGE_FIXUP_STAT = "monster_base_type_attack_cast_speed_+%_and_damage_-%_final"

function MercenaryTools.monsterSpeedAndDamageFixup(modId, stats)
	if type(modId) ~= "string" or not modId:find("SpeedAndDamageFixup", 1, true) then
		return nil
	end
	for _, stat in ipairs(stats or { }) do
		if stat.id == MercenaryTools.MONSTER_SPEED_DAMAGE_FIXUP_STAT and tonumber(stat.value) then
...
function MercenaryTools.considerRankedCandidate(current, id, rank)
...
function MercenaryTools.requireUniqueRankedCandidate(current, context)
	if not current then
		return nil
	end
...
function MercenaryTools.shieldPolicyError(builds, shieldPolicy)
