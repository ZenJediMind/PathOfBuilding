...
do
	local out = assert(io.open("../Data/Skills/mercenary.lua", "w"))
	out:write("-- This file is automatically generated, do not edit!\n")
	out:write("-- Mercenary skill data (c) Grinding Gear Games\n\nlocal skills, mod, flag, skill = ...\n\n")
	out:write("local existingSkills = { }\n")
	out:write("for skillId in pairs(skills) do existingSkills[skillId] = true end\n\n")
	out:write("local function inheritSkillData(skillId, baseSkillId)\n")
	out:write("\tlocal target, base = skills[skillId], skills[baseSkillId]\n")
	out:write("\tif not base then error(\"Missing base skill mapping: \"..baseSkillId..\" for \"..skillId) end\n")
	out:write("\tlocal function inherit(source)\n")
	out:write("\t\tfor key, value in pairs(source) do\n")
	out:write("\t\t\tif target[key] == nil then\n")
	out:write("\t\t\t\ttarget[key] = type(value) == \"table\" and copyTable(value, true) or value\n")
	out:write("\t\t\telseif key == \"baseFlags\" then\n")
	out:write("\t\t\t\tfor flag, enabled in pairs(value) do if target.baseFlags[flag] == nil then target.baseFlags[flag] = enabled end end\n")
	out:write("\t\t\tend\n")
	out:write("\t\tend\n")
	out:write("\tend\n")
	out:write("\tlocal matchingSkillIds = { }\n")
	out:write("\tfor candidateId, candidate in pairs(skills) do\n")
	out:write("\t\tlocal sameSkill = candidate.name == target.name or target.description and target.description ~= \"\" and candidate.description == target.description\n")
	out:write("\t\tif candidateId ~= skillId and candidateId ~= baseSkillId and candidate.baseTypeName and not candidate.hidden and sameSkill then\n")
	out:write("\t\t\ttable.insert(matchingSkillIds, candidateId)\n")
	out:write("\t\tend\n")
	out:write("\tend\n")
	out:write("\ttable.sort(matchingSkillIds, function(leftId, rightId)\n")
	out:write("\t\tlocal left, right = skills[leftId], skills[rightId]\n")
	out:write("\t\tlocal leftPriority = left.name == target.name and 0 or 1\n")
	out:write("\t\tlocal rightPriority = right.name == target.name and 0 or 1\n")
	out:write("\t\treturn leftPriority == rightPriority and leftId < rightId or leftPriority < rightPriority\n")
	out:write("\tend)\n")
	out:write("\tfor _, candidateId in ipairs(matchingSkillIds) do inherit(skills[candidateId]) end\n")
	out:write("\tinherit(base)\n")
	out:write("end\n\n")
	local emitted = { }
	for _, templateName in ipairs({"act_str","act_dex","act_int","other","glove","minion","spectre","sup_str","sup_dex","sup_int"}) do
		for line in io.lines("Skills/"..templateName..".txt") do
			local id = line:match("^#skill%s+(%S+)")
			if id then
				emitted[id] = true
			end
		end
	...
	for id in pairs(emitted) do
		...
		if effect and effect.ActiveSkill then
			local activeSkillId = effect.ActiveSkill.Id
			if not baseSkillByActiveSkill[activeSkillId] or id < baseSkillByActiveSkill[activeSkillId] then
				baseSkillByActiveSkill[activeSkillId] = id
			end
			local displayName = effect.ActiveSkill.DisplayName
			if displayName ~= "" and (not baseSkillByDisplayName[displayName] or id < baseSkillByDisplayName[displayName]) then
				baseSkillByDisplayName[displayName] = id
			end
		end
	end
	local flagBySkillType = {
		Attack = "attack",
		Spell = "spell",
		Area = "area",
	...
	local rows = { }
	for row in dat("MercenarySkills"):Rows() do
		table.insert(rows, row)
	end
	table.sort(rows, function(a, b) return a.Id.Id < b.Id.Id end)
	local effectNames, effects = { }, { }
	for _, row in ipairs(rows) do
		for _, effect in ipairs({ row.Id, row.SecondaryGrantedEffect }) do
			if effect then
				effects[effect.Id] = effect
				effectNames[effect.Id] = row.Name ~= "" and row.Name or effect.ActiveSkill.DisplayName ~= "" and effect.ActiveSkill.DisplayName or effect.Id
				for index, stat in ipairs(effect.GrantedEffectStatSets.ConstantStats or { }) do
					if stat.Id == "alternate_minion" then
						local summonedId = effect.GrantedEffectStatSets.ConstantStatsValues[index]
						...
						for _, minionEffect in ipairs(variety.GrantedEffects or { }) do
							effects[minionEffect.Id] = minionEffect
							effectNames[minionEffect.Id] = minionEffect.ActiveSkill.DisplayName ~= "" and minionEffect.ActiveSkill.DisplayName or minionEffect.Id
						end
					end
				end
			end
	...
	for _, effectId in ipairs(effectIds) do
		...
		if not emitted[effect.Id] then
				emitted[effect.Id] = true
				local state = { noGem = true }
				directiveTable.skill(state, effect.Id.." "..effectNames[effect.Id], out)
				local flags = { }
				for _, skillType in ipairs(effect.ActiveSkill.SkillTypes) do
					if flagBySkillType[skillType.Id] then
						flags[flagBySkillType[skillType.Id]] = true
					end
				end
				local orderedFlags = { }
				for flagName in pairsSortByKey(flags) do
					table.insert(orderedFlags, flagName)
				end
				directiveTable.flags(state, table.concat(orderedFlags, " "), out)
				directiveTable.mods(state, "", out)
				local baseSkillId = baseSkillByActiveSkill[effect.ActiveSkill.Id] or baseSkillByDisplayName[effect.ActiveSkill.DisplayName] or baseSkillByDisplayName[effectNames[effect.Id]]
				if baseSkillId then
					out:write("\ninheritSkillData(\"", effect.Id, "\", \"", baseSkillId, "\")")
				end
				out:write("\n\n")
		end
	end
	out:write("for skillId, grantedEffect in pairs(skills) do\n")
	out:write("\tif not existingSkills[skillId] then\n")
	out:write("\t\tgrantedEffect.mercenary = true\n")
	out:write("\t\tfor _, statId in ipairs(grantedEffect.stats or { }) do\n")
	out:write("\t\t\tif statId == \"base_is_projectile\" then grantedEffect.baseFlags.projectile = true end\n")
	out:write("\t\t\tif statId == \"is_area_damage\" then grantedEffect.baseFlags.area = true end\n")
	out:write("\t\t\tif statId:match(\"^spell_minimum_base_.*_damage$\") then grantedEffect.baseFlags.hit = true end\n")
	out:write("\t\tend\n")
	out:write("\tend\n")
	out:write("end\n")
	out:close()
