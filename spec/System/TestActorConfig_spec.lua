describe("Player and mercenary configuration", function()
	...
	it("switching Config Sets does not change actor equipment", function()
		...
		configTab:Save(xml)
		-- Player and shared values keep the upstream ConfigSet layout; only the Mercenary gets an Actor node.
		local actorIds, mercenaryMods, playerMods, sharedBoss = { }, nil, nil, nil
		...
		assert.is_nil(actorIds.player)
		assert.is_not_nil(actorIds.mercenary)
		assert.are.equal("10% increased Damage", playerMods)
		...
		assert.are.equal("10% increased Damage", loaded.customModsList[1].text)
