...
directiveTable.skill = function(state, args, out)
	...
	if state.mercenary then
		out:write('\tmercenary = true,\n')
		if state.mercenaryBaseSkillId then
			out:write('\tinheritedFrom = "', state.mercenaryBaseSkillId, '",\n')
		end
	end
	state.mercenary = nil
	state.mercenaryBaseSkillId = nil
...
do
	-- Read the last generated file before truncating it. Opening for write
	-- first would empty previousInheritedFrom and fail closed on shared
	-- ActiveSkills such as geometry_spell.
	...
	-- A Mercenary Granted Effect reuses the implementation of the player skill it
	-- was derived from. Resolve that base here, from the shared ActiveSkill where
	-- possible and from the shared display name otherwise, and emit it as
	...
	local function baseRank(effect)
		if effect.IsSupport then return 2 end
		local gemEffect = dat("GemEffects"):GetRow("GrantedEffect", effect) or dat("GemEffects"):GetRow("GrantedEffect2", effect)
	...
	for id in pairs(emitted) do
		...
		if effect and effect.ActiveSkill then
			local rank = baseRank(effect)
			considerBase(baseSkillByActiveSkill, effect.ActiveSkill.Id, id, rank)
			local displayName = effect.ActiveSkill.DisplayName
			if displayName ~= "" then
				considerBase(baseSkillByDisplayName, displayName, id, rank)
			end
		end
	end
	...
	for _, effectId in ipairs(effectIds) do
		...
		if not emitted[effect.Id] then
				...
				local byActiveSkill = baseSkillByActiveSkill[effect.ActiveSkill.Id]
