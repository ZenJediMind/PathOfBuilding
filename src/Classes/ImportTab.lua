...
function ImportTabClass:ImportItemsAndSkills(charData, clearItems, clearSkills, ignoreWeaponSwap)
	...
	self.build.itemsTab:PopulateSlots()
	self.build.itemsTab:SetViewItemSet(self.build.itemsTab.activeItemSetId, "PLAYER")
	self.build.itemsTab:AddUndoState()
...
function ImportTabClass:ImportItem(itemData, slotName, ignoreWeaponSwap, itemSetId, context)
	...
	if item.base then
		...
		if itemSetId then
			...
			local targetSlot = assert(itemSet[slotName], "Missing imported item slot: "..slotName)
...
function ImportTabClass:ImportSocketedItems(item, socketedItems, slotName, itemSetId, context)
	...
	local mercenaryOccupancy = context and context.actor == "MERCENARY"
	if mercenaryOccupancy then
		item.socketedGems = item.socketedGems or { }
	end
	for _, socketedItem in ipairs(socketedItems) do
		if socketedItem.abyssJewel then
			...
			if gemId then
				...
				for _, property in pairs(socketedItem.properties or { }) do
				...
				if mercenaryOccupancy then
					-- Preserve occupancy for socket-property item bonuses; do not create usable gem skills.
					item.socketedGems[(socketedItem.socket or 0) + 1] = {
						gemId = gemInstance.gemId,
						nameSpec = gemInstance.nameSpec,
						support = gemInstance.support,
...
function ImportTabClass:OpenMercenaryImportPopup(account, source, current)
	...
	local function selectHire(_, selected)
		...
		for _, id in ipairs(tab.mercenarySetOrderList) do
			...
			local matchesSource = MercenaryImport.matchesAssociation(profile.importAssociation, source.account, source.realm, source.league, selected.hire)
...
function ImportTabClass:ImportMercenary(hire, source, destinationId)
	...
	if destinationId == nil then
		for _, id in ipairs(tab.mercenarySetOrderList) do
			if MercenaryImport.matchesAssociation(tab.mercenarySets[id].importAssociation, source.account, source.realm, source.league, hire) then
	...
	for _, item in pairs(context.staged) do
		if MercenaryTools.playerEquipsPhysicalItem(item, itemsTab.activeItemSet, itemsTab.items) then
			return nil, "This item is still equipped by the player in PoB; refresh the character import or unequip it first."
		end
	end

	-- Replace the destination loadout's own equipment set. Fork only if that
	-- would overwrite the player's currently equipped gear.
	local previousSetId = previous and previous.itemSetId
	...
	-- Bind the active Config only when creating a hire or filling an unused empty
	-- slot (including an explicit empty destination). Reimport, and explicit
	-- overwrite of an occupied slot with a different hire, must not steal the
	-- active loadout's binding.
	local bindToLoadout = not previous or (not previous.buildId and not previous.importAssociation)
	tab:SetActiveMercenarySet(profile.id, false, bindToLoadout)
	tab:AddUndoState()
