	...
	self:NewSection(3, "SkillSelect", 1, colorCodes.NORMAL, {{ defaultCollapsed = false, label = "View Skill Details", data = {
		...
		{ label = "Calculation Actor", { controlName = "actor",
			control = new("DropDownControl"):DropDownControl(nil, {0, 0, 160, 16}, nil, function(index, value)
				self:SetCalculationActor(value.actorId)
				self:AddUndoState()
				self.build.buildFlag = true
			end)
		}, },
...
Effective DPS: Curses and enemy properties (such as resistances and status conditions) will also apply. This estimates your true DPS.]]) 
		...
		local minionList = section.controls.mainSkillMinion and section.controls.mainSkillMinion.list
		local usesItemSet = minionList and minionList[1] and minionList[1].itemSetId
		for _, row in ipairs(section.subSection[1].data) do
			if row[1] and row[1].controlName == "mainSkillMinion" then
				row.label = usesItemSet and "Item Set" or "Minion"
				break
			end
		end
	end)
...
function CalcsTabClass:Load(xml, dbFileName)
	...
	if not savedActor then
		self.input.legacyShowMinion = not not self.input.showMinion
...
function CalcsTabClass:SetCalculationActor(actorId)
	self.input.actor = actorId
	self.input.legacyShowMinion = nil
end
...
function CalcsTabClass:GetDisplayActor(env)
	...
	local actor = actors[self.input.actor or "PLAYER"]
	if actor then
		return actor
	end
	-- Only the showMinion XML migration falls back; an explicit Player Minion
	-- selection stays unavailable when this build has no minion.
	if self.input.legacyShowMinion and self.input.actor == "PLAYER_MINION" then
		return env.player
	end
end
